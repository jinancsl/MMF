...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 7] + sheets2[j_9, 13] + sheets2[j_9, 19]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 7] + sheets2[j_9, 13] + sheets2[j_9, 19]) <= 0.05:
...
def main():
    ...
    for chongfu in range(5):
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_gugu_wai_hou().cuda()
