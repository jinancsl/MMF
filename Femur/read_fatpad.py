...
def people_label():
    excel_label_dir = '/media/ExtHDD02/csl/methods_data/biyelunwen/twentyninefenqu/dataset/fenlei29fenqu/gugu.xlsx'
    book0 = pandas.read_excel(excel_label_dir, engine='openpyxl', sheet_name='MOAKS_sin_left')
    sheets0 = book0.values
    #sheets0 = np.delete(sheets0, 2, axis=1)

    book1 = pandas.read_excel(excel_label_dir, engine='openpyxl', sheet_name='MOAKS_mul_left')
    sheets1 = book1.values
    #sheets1 = np.delete(sheets1, 2, axis=1)


    book2 = pandas.read_excel(excel_label_dir, engine='openpyxl', sheet_name='MOAKS_sin_right')
    sheets2 = book2.values
    #sheets2 = np.delete(sheets2, 2, axis=1)

    book3 = pandas.read_excel(excel_label_dir, engine='openpyxl', sheet_name='MOAKS_mul_right')
