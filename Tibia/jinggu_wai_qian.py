...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 4] + sheets2[j_9, 11] + sheets2[j_9, 18]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 4] + sheets2[j_9, 11] + sheets2[j_9, 18]) <= 0.05:
...
def main():
    ...
    np.savez(os.path.join(
        ...
        'train_jinggu_wai_qian'), image=train_dataset, label=train_label)  # 保存乱序后的数据集
    ...
    np.savez(os.path.join(
        ...
        'test_jinggu_wai_qian'), image=test_dataset, label=test_label)  # 保存乱序后的数据集
    ...
    np.savez(os.path.join(
        ...
        'validate_jinggu_wai_qian'), image=validate_dataset, label=validate_label)  # 保存乱序后的数据集
    ...
    datasets_jinggu_train = np.load(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/jinggu/train/',
        'train_jinggu_wai_qian.npz'), allow_pickle=True)  # 加载train datasets
    ...
    datasets_jinggu_test = np.load(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/jinggu/test/',
        'test_jinggu_wai_qian.npz'), allow_pickle=True)  # 加载train datasets
    ...
    datasets_jinggu_train_nei = np.load(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/jinggu/validate/',
        'validate_jinggu_wai_qian.npz'), allow_pickle=True)  # 加载train datasets
    ...
    for chongfu in range(10):
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_jinggu_wai_qian().cuda()
        ...
        for epoch in range(args.epochs):
            ...
            if min(avg_acc_val[1], avg_acc_val[2]) > max_auc:
                ...
                torch.save(net, '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/jinggu/' + str(chongfu) + 'jinggu_wai_qian.pth')
        ...
        model = torch.load('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/jinggu/' + str(chongfu) + 'jinggu_wai_qian.pth')  # 模型
        model = model.cuda()
        batch = 4
        ...
        np.savez(os.path.join('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/result/jinggu/AUC/',
            'jinggu_wai_qian' + str(chongfu)), all_out_test=all_out_test, all_label_test=all_label_test)  # 保存结果和标签
        plt.savefig('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/result/jinggu/AUC/jinggu_wai_qian' + str(chongfu) + '.jpg', dpi=300)
