...
def read_niigz_fenge_label(file_path):
    ...
    image_arr_all_temp[image_arr_all == 8] = 1
    image_arr_all_temp[image_arr_all == 9] = 1
...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 3] + sheets2[j_9, 5] + sheets2[j_9, 7] + sheets2[j_9, 10] + sheets2[j_9, 12] + sheets2[j_9, 14] + sheets2[j_9, 17] + sheets2[j_9, 19] + sheets2[j_9, 21]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 3] + sheets2[j_9, 5] + sheets2[j_9, 7] + sheets2[j_9, 10] + sheets2[j_9, 12] + sheets2[j_9, 14] + sheets2[j_9, 17] + sheets2[j_9, 19] + sheets2[j_9, 21]) <= 0.05:
...
def main():
    ...
    '''
    '''
    datasets_jinggu_validate = np.load(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/jinggu/validate/',
        'validate_' + zuzi + '.npz'), allow_pickle=True)  # 加载train datasets
    validate_dataset = datasets_jinggu_validate['image']
    validate_label = datasets_jinggu_validate['label']
    '''
    for chongfu in range(3):
        args = parse_args()
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_jinggu_nei().cuda()
        ...
        #model = model.cuda()
        #model = nn.DataParallel(model)  # 指定显卡
