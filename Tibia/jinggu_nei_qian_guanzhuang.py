...
def jietu(jixian_people, size1, size2, image_people_temp, fenge_label):
    ...
    if jixian_people_zuobiao[3] >= image_people_temp.shape[2]:
        jixian_people_zuobiao[3] = image_people_temp.shape[2]
        jixian_people_zuobiao[2] = jixian_people_zuobiao[3] - size2

    zong_temp = []
    for u_10 in range(fenge_label.shape[0]):
        if fenge_label[u_10, :, :].max() == 0:
            image_people_temp[u_10, :, :] = np.zeros([image_people_temp.shape[1], image_people_temp.shape[2]])
        else:
...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        image_people = image_people.transpose(2, 1, 0)
        label_people = label_people.transpose(2, 1, 0)
        ###################读取标签
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 3] + sheets2[j_9, 10] + sheets2[j_9, 17]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 3] + sheets2[j_9, 10] + sheets2[j_9, 17]) <= 0.05:
...
def main():
    zuzi = 'jinggu_nei_qian_guanzhuang'
    ...
    for chongfu in range(10):
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_jinggu_nei_qian().cuda()
