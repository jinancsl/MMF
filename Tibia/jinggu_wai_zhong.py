...
def read_niigz_fenge_label(file_path):
    ...
    '''
    #image_arr_all_temp[image_arr_all == 1] = 1
    image_arr_all_temp[image_arr_all == 2] = 1
    #image_arr_all_temp[image_arr_all == 3] = 1
...
def jietu(jixian_people, size1, size2, image_people_temp, fenge_label):
    ...
    for u_9 in range(len(zong_temp)):
        ...
        #image_people[u_9, :, :] = cv2.resize(zong_temp[u_9], (96, 96), interpolation=cv2.INTER_NEAREST)
        image_people[u_9, :, :] = zong_temp[u_9]
        #image_people[u_9, :, :] = Normalize_image(image_people[u_9, :, :])
...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 6] + sheets2[j_9, 13] + sheets2[j_9, 20]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 6] + sheets2[j_9, 13] + sheets2[j_9, 20]) <= 0.05:
        ...
        if jixian_people[3] - jixian_people[2] > size2:
            chazhi = int((jixian_people[3] - jixian_people[2] - size2) / 2)
            jixian_people[2] = jixian_people[2] + chazhi
            jixian_people[3] = jixian_people[2] + size2
            print('taida', jixian_people, i[:-7])

        if jixian_people[1] - jixian_people[0] > size1:
            jixian_people[0] = jixian_people[0] - 10
            jixian_people[1] = jixian_people[0] + size1
            print('taida', jixian_people, i[:-7])
        ...
        pianyi = 10
        shuzhou_xunhuan = int((size1 - (jixian_people[1] - jixian_people[0])) / (2 * pianyi))
        #hengzhou_xunhuan = int((size2 - (jixian_people[3] - jixian_people[2])) / (2 * pianyi))
        if shuzhou_xunhuan > 1 and labels_batch_ndarry[0] == 1:
            for shuzhou_jishu in range(shuzhou_xunhuan - 1):
                jixian_people_shuzhou = jixian_people.copy()
                jixian_people_shuzhou[1] = min(384, jixian_people_shuzhou[1] + ((shuzhou_jishu + 1) * pianyi + random.randint(-5, 5)))
                ...
                jixian_people_shuzhou = jixian_people.copy()
                jixian_people_shuzhou[0] = jixian_people_shuzhou[0] - ((shuzhou_jishu + 1) * pianyi + random.randint(-5, 5))
                image_shuzhou = jietu(jixian_people_shuzhou, size1, size2, image_people, label_people)
                zong.append(image_shuzhou)
                labels_batch_ndarry_zong[len(zong) - 1, 0] = labels_batch_ndarry[0]
                name.append(i[:-7])
    return zong, labels_batch_ndarry_zong, name

def main():
    zuzi = 'jinggu_wai_zhong'
    ...
    '''
    train_dataset_temp, train_label_temp, name_train = datasets_loading(files_slice_train, data_dir_train, data_dir_train_label)
    zhengxu = list(range(len(train_dataset_temp)))
    random.shuffle(zhengxu)
    train_dataset = []
    train_label = np.zeros([len(train_dataset_temp), 1])
    ...
    np.savez(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/jinggu/train/',
        'train_' + zuzi), image=train_dataset, label=train_label)  # 保存乱序后的数据集
    del train_dataset_temp, train_label_temp, zhengxu
    gc.collect()

    test_dataset_temp, test_label_temp, name_test = datasets_loading(files_slice_test, data_dir_test, data_dir_test_label)
    zhengxu = list(range(len(test_dataset_temp)))
    random.shuffle(zhengxu)
    test_dataset = []
    test_label = np.zeros([len(test_dataset_temp), 1])
    test_name = []
    for j in range(len(test_dataset_temp)):
        ...
        test_name.append(name_test[zhengxu[j]])
    np.savez(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/jinggu/test/',
        'test_' + zuzi), image=test_dataset, label=test_label, name=test_name)  # 保存乱序后的数据集
    del test_dataset_temp, test_label_temp, zhengxu
    gc.collect()
    '''
    '''
    validate_dataset_temp, validate_label_temp, name_validate = datasets_loading(files_slice_validate, data_dir_validate, data_dir_validate_lable)
    zhengxu = list(range(len(validate_dataset_temp)))
    random.shuffle(zhengxu)
    validate_dataset = []
    validate_label = np.zeros([len(validate_dataset_temp), 1])
    ...
    np.savez(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/jinggu/validate/',
    ...
    test_name = datasets_jinggu_test['name']
    ...
    for chongfu in range(20):
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_jinggu_wai_zhong().cuda()
        ...
        model = model.cuda()
        #model = torch.load('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/jinggu/' + str(
        #    chongfu) + 'jinggu_nei_zhong' + '.pth')  # 模型
        ...
        for epoch in range(args.epochs):
            ...
            avg_loss_val, sum_shuju_val, crect_sum_shuju_val, auc_val = val(net, criterion, test_dataset,
                                                                                   test_label, batch)
            #scheduler.step()
            ...
            if min(avg_acc_val[1], avg_acc_val[2]) > max_auc:
                ...
                torch.save(net, '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/jinggu/' + str(chongfu) + zuzi + '.pth')
        ...
        model = torch.load('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/jinggu/' + str(chongfu) + zuzi + '.pth')  # 模型
        model = model.cuda()
        batch = 1
        ...
        # 6.展示图片和保存
        np.savez(os.path.join('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/result/jinggu/AUC/',
            zuzi + str(chongfu)), all_out_test=all_out_test, all_label_test=all_label_test, name=test_name)  # 保存结果和标签
        plt.savefig('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/result/jinggu/AUC/' + zuzi + str(chongfu) + '.jpg', dpi=300)
