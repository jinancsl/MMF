...
def read_niigz_fenge_label(file_path):
    ...
    image_arr_all_temp[image_arr_all == 13] = 1
...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 2] + sheets2[j_9, 9] + sheets2[j_9, 16]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 2] + sheets2[j_9, 9] + sheets2[j_9, 16]) <= 0.05:
        ...
        if shuzhou_xunhuan > 1 and labels_batch_ndarry[0] == 1:
            for shuzhou_jishu in range(shuzhou_xunhuan - 1):
                ...
                jixian_people_shuzhou[0] = jixian_people_shuzhou[0] - (
                            (shuzhou_jishu + 1) * pianyi + random.randint(-2, 3))
        ...
        if hengzhou_xunhuan > 1 and labels_batch_ndarry[0] == 1:
            for hengzhou_jishu in range(hengzhou_xunhuan - 1):
                jixian_people_hengzhou = jixian_people.copy()
                jixian_people_hengzhou[3] = jixian_people_hengzhou[3] + ((hengzhou_jishu + 1) * pianyi + random.randint(-2, 3))
                ...
                jixian_people_hengzhou = jixian_people.copy()
                jixian_people_hengzhou[2] = jixian_people_hengzhou[2] - ((hengzhou_jishu + 1) * pianyi + random.randint(-2, 3))
                image_hengzhou = jietu(jixian_people_hengzhou, size1, size2, image_people, label_people)
                zong.append(image_hengzhou)
                labels_batch_ndarry_zong[len(zong) - 1, 0] = labels_batch_ndarry[0]
                name.append(i[:-7])
...
def main():
    ...
    '''
    train_dataset_temp, train_label_temp, name_train = datasets_loading(files_slice_train, data_dir_train,
                                                                        data_dir_train_label)
    ...
    test_dataset_temp, test_label_temp, name_test = datasets_loading(files_slice_test, data_dir_test,
                                                                     data_dir_test_label)
    ...
    for chongfu in range(10):
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_wuruangufugaiqu().cuda()
        ...
        for epoch in range(args.epochs):
            ...
            if min(avg_acc_val[1], avg_acc_val[2]) > max_auc:
                ...
                torch.save(net, '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/jinggu/' + str(
                    chongfu) + zuzi + '.pth')
        ...
        model = model.cuda()
        batch = 1
        avg_loss_test, sum_shuju_test, crect_sum_shuju_test, auc_test, all_label_test, all_out_test = test(model,
                                                                                                           criterion,
                                                                                                           test_dataset,
                                                                                                           test_label,
                                                                                                           batch)
        ...
        plt.savefig('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/result/jinggu/AUC/' + zuzi + str(
            chongfu) + '.jpg', dpi=300)
