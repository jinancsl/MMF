...
NII_train_jinggu_fenlei = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/smu_dataset/fenlei/zhengli/train'
NII_train_jinggu_fenlei_validate = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/smu_dataset/fenlei/zhengli/validate'
NII_train_jinggu_fenlei_test = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/smu_dataset/fenlei/zhengli/test'

data_dir = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/my/segmention_for_classification/femur/train/image'
data_dir_lable = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/my/segmention_for_classification/femur/train/label'

data_dir_validate = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/my/segmention_for_classification/femur/validate/image'
data_dir_validate_lable = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/my/segmention_for_classification/femur/validate/label'

data_dir_test = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/my/segmention_for_classification/femur/test/image'
data_dir_test_label = '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/knee_dataset/my/segmention_for_classification/femur/test/label'
...
def read_niigz_fenge_label(file_path):
    ...
    '''
    image_arr_all_temp[image_arr_all == 7] = 1
    return image_arr_all_temp

def datasets_loading(files_people, NII_dir_1, NII_dir_2, NII_dir_3):
    files = os.listdir(NII_dir_1)    #列出所有的用于训练的数据
    image_label_files = []
    for files_i in files:
        image_files = os.listdir(os.path.join(NII_dir_1, files_i))
        for files_people_temp in image_files:
            image_label_files.append(os.path.join(NII_dir_1, files_i, files_people_temp))
    ...
    for i in files_people:
        for j in range(len(image_label_files)):
            if str(i[0:7]) == image_label_files[j][-7:]:
                dir_people = image_label_files[j]

        #####读取用于训练的三维图像
        if os.path.exists(os.path.join(dir_people,
                                       str(i[0:7]) + '-00m-SAG_3D_DESS_RIGHT')):  # 如果没有这个人，则跳过这一次循环，否则读取相应的序列
            save_name = str(i[0:7]) + '-00m-SAG_3D_DESS_RIGHT'
        elif os.path.exists(os.path.join(dir_people,
        ...
        for j_9 in range(len(sheets3)):
            if save_name == sheets3[j_9]:
                if sheets2[j_9, 5] > 0.5:
                    labels_batch_ndarry[0] = 1
                elif sheets2[j_9, 5] == 0:
...
def main():
    ...
    test_dataset, test_label = datasets_loading(files_slice_test, NII_train_jinggu_fenlei_test, data_dir_test, data_dir_test_label)
    validate_dataset, validate_label = datasets_loading(files_slice_validate, NII_train_jinggu_fenlei_validate, data_dir_validate, data_dir_validate_lable)
    
    np.savez(os.path.join('/media/chen/DATA2/knee_dataset/my/bone_cartilage/classification/femoral_cartliage', 'train_ruangu_wai_qian'), image=train_dataset, label=train_label)  # 保存乱序后的数据集
    np.savez(os.path.join('/media/chen/DATA2/knee_dataset/my/bone_cartilage/classification/femoral_cartliage', 'test_ruangu_wai_qian'), image=test_dataset, label=test_label)  # 保存乱序后的数据集
    np.savez(os.path.join('/media/chen/DATA2/knee_dataset/my/bone_cartilage/classification/femoral_cartliage', 'validate_ruangu_wai_qian'), image=validate_dataset, label=validate_label)  # 保存乱序后的数据集
    '''
    datasets_jinggu_train = np.load(os.path.join('/media/chen/DATA2/knee_dataset/gu_gusui/fenlei/dataset_my/gugu/maked/','train_datasets_guguruangu_nei_qian.npz'), allow_pickle=True)  # 加载train datasets
    train_dataset = datasets_jinggu_train['image']
    train_label = datasets_jinggu_train['label']
    datasets_jinggu_test = np.load(os.path.join('/media/chen/DATA2/knee_dataset/gu_gusui/fenlei/dataset_my/gugu/maked/','test_datasets_guguruangu_nei_qian.npz'), allow_pickle=True)  # 加载train datasets
    test_dataset = datasets_jinggu_test['image']
    test_label = datasets_jinggu_test['label']
    datasets_jinggu_train_nei = np.load(os.path.join('/media/chen/DATA2/knee_dataset/gu_gusui/fenlei/dataset_my/gugu/maked/','validate_datasets_guguruangu_nei_qian.npz'), allow_pickle=True)  # 加载train datasets
    ...
    for epoch in range(args.epochs):
        ...
        net, avg_loss, cum_acc_crect_train, cum_acc_train, auc_train = train(model, optimizer, criterion, train_dataset, train_label)

        val_avg_loss, cum_acc_crect_validate, cum_acc_validate, auc_test = val(net, criterion, validate_dataset, validate_label)
        ...
        if (acc_single[0] + acc_single[1])/2 > max_auc:
            zhongduan = 0
            max_auc = (acc_single[0] + acc_single[1])/2
