...
def read_niigz_fenge_label(file_path):
    ...
    '''
    image_arr_all_temp[image_arr_all == 23] = 1
...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 4]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 4]) <= 0.05:
...
def main():
    ...
    for chongfu in range(5):
        if chongfu == 0 or chongfu == 1 or chongfu == 3:
            continue
        args = parse_args()
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_guguruangu_nei_qian().cuda()
