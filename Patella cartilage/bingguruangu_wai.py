...
def read_niigz_fenge_label(file_path):
    ...
    '''
    image_arr_all_temp[image_arr_all == 28] = 1
...
def datasets_loading(files_people, data_dir_image, data_dir_label):
    ...
    for i in files_people:
        ...
        for j_9 in range(len(sheets3)):
            if i[:-7] == sheets3[j_9]:
                if (sheets2[j_9, 3]) > 0.1:
                    labels_batch_ndarry[0] = 1
                elif (sheets2[j_9, 3]) <= 0.05:
...
def main():
    ...
    np.savez(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/binguruangu/train/',
    ...
    np.savez(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/binguruangu/test/',
    ...
    np.savez(os.path.join(
        '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/dataset/binguruangu/validate/',
    ...
    for chongfu in range(5):
        ...
        if args.loss == 'BCEWithLogitsLoss':
            ...
            criterion = losses.BCEFocalLosswithLogits_bingguruangu_wai().cuda()
        ...
        for epoch in range(args.epochs):
            ...
            if min(avg_acc_val[1], avg_acc_val[2]) > max_auc:
                ...
                torch.save(net, '/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/binguruangu/' + str(chongfu) + zuzi + '.pth')
        ...
        model = torch.load('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/model/binguruangu/' + str(chongfu) + zuzi + '.pth')  # 模型
        model = model.cuda()
        batch = 4
        ...
        # 6.展示图片和保存
        np.savez(os.path.join('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/result/binguruangu/AUC/',
            zuzi + str(chongfu)), all_out_test=all_out_test, all_label_test=all_label_test)  # 保存结果和标签
        plt.savefig('/media/ExtHDD02/csl/methods_data/biyelunwen/twentynine_clc/result/binguruangu/AUC/' + zuzi + str(chongfu) + '.jpg', dpi=300)
